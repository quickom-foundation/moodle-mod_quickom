 ...
 * @package    mod_quickom
 * @copyright  2020 Beowulf Blockchain.
 * @copyright  based on work by2018 UC Regents
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class get_meeting_reports extends \core\task\scheduled_task {
     ...
     * @link https://quickom.github.io/api/#report-metric-apis
     */
    public function execute() {
        ...
        $starttime = get_config('mod_quickom', 'last_call_made_at');

        // Quickom requires this format when passing the to and from arguments.
        // Quickom just returns all the meetings from the day range instead of actual time range specified.
        ...
        $cclehostrecords = $DB->get_records('quickom', null, '', 'id, host_id');
        $cclehosts = array();
        ...
        foreach ($activehostsuuids as $activehostsuuid) {
            ...
            if ($this->get_num_calls_left() < 1) {
                ...
                mtrace('Error: Quickom Report API calls have been exhausted.');
                return;
            }
        }
        ...
        foreach ($allmeetings as $meeting) {
            if (!($DB->record_exists('quickom_meeting_details', array('uuid' => $meeting->uuid)))) {
                // If meeting doesn't exist in the quickom database, the instance is deleted, and we don't need reports for these.
                if (!($quickomrecord = $DB->get_record('quickom', array('meeting_id' => $meeting->id), '*', IGNORE_MULTIPLE))) {
                    continue;
                }

                $meeting->quickomid = $quickomrecord->id;
                ...
                if ($this->get_num_calls_left() < 1) {
                    ...
                    set_config('last_call_made_at', $meeting->start_time - 1, 'mod_quickom');
                    ...
                    // According to Quickom support, this is the case.
                    break;
                }
                $detailsid = $DB->insert_record('quickom_meeting_details', $meeting);
                $iswebinar = $quickomrecord->webinar;
                $participants = $service->get_meeting_participants($meeting->uuid, $iswebinar);

                // Loop through each user to generate name->uids mapping.
                list($names, $emails) = $this->get_enrollments($quickomrecord->course);

                foreach ($participants as $rawparticipant) {
                    $participant = $this->format_participant($rawparticipant, $detailsid, $names, $emails);
                    $DB->insert_record('quickom_meeting_participants', $participant, false);
                }
            }
        }
        if ($recordedallmeetings) {
            set_config('last_call_made_at', time(), 'mod_quickom');
        }
    }
     ...
     * @param int $detailsid The id to link to the quickom_meeting_details table.
    ...
    public function format_participant($participant, $detailsid, $names, $emails) {
        ...
        if (!empty($participant->id)) {
            // Sometimes uuid is blank from Quickom.
            $participantmatches = $DB->get_records('quickom_meeting_participants',
        ...
        if (empty($moodleuserid)) {
            if (!empty($participant->user_email) && ($moodleuserid =
                ...
                // Did not find any matches, so use what is given by Quickom.
        ...
        return array(
            ...
            'userid' => $moodleuserid,
            'detailsid' => $detailsid,
            'quickomuserid' => $participant->user_id,
            'uuid' => $participant->id,
    ...
    public function get_name() {
        return get_string('getmeetingreports', 'mod_quickom');
    }
    ...
    public function get_num_calls_left() {
        return get_config('mod_quickom', 'calls_left');
    }
