...
$PAGE->set_url('/mod/quickom/participants.php', array('id' => $cm->id, 'uuid' => $uuid, 'export' => $export));

$strname = $quickom->name;
$strtitle = get_string('participants', 'mod_quickom');
...
if (empty($export) || empty($participants)) {
    ...
    if (empty($participants)) {
        notice(get_string('noparticipants', 'mod_quickom'),
                new moodle_url('/mod/quickom/report.php', array('id' => $cm->id)));
        echo $OUTPUT->footer();
        exit();
    }
}
...
if (!empty($export)) {
    $table->head = array(get_string('idnumber'),
                         get_string('name'),
                         get_string('email'),
                         get_string('jointime', 'mod_quickom'),
                         get_string('leavetime', 'mod_quickom'),
                         get_string('duration', 'mod_quickom'),
                         get_string('attentiveness_score', 'mod_quickom'));
} else {
    $table->head = array(get_string('idnumber'),
                         get_string('name'),
                         get_string('jointime', 'mod_quickom'),
...
if ($export != 'xls') {
    ...
    echo html_writer::tag('p', get_string('attentiveness_score_help', 'quickom'));

    $exporturl = new moodle_url('/mod/quickom/participants.php', array(
            'id' => $cm->id,
            'uuid' => $uuid,
            'export' => 'xls'
        ));
    ...
    echo get_string('export', 'mod_quickom') . ': ' . html_writer::link($exporturl, $xlsicon);

    echo $OUTPUT->footer();
} else {
    require_once(dirname(__FILE__).'/../../lib/excellib.class.php');

    $workbook = new MoodleExcelWorkbook("quickom_participants_{$quickom->meeting_id}");
