...
function populate_quickom_from_response(stdClass $quickom, stdClass $response)
{
    ...
    $samefields = array('start_url', 'join_url', 'created_at', 'timezone', 'creator_id', 'host_key', 'alias');
...
function quickom_delete_instance($id)
{
    ...
    $DB->delete_records('quickom', array('id' => $quickom->id));

    // If we delete a meeting instance, do we want to delete the participants?
    $meetinginstances = $DB->get_records('quickom_meeting_details', array('meeting_id' => $quickom->meeting_id));
    foreach ($meetinginstances as $meetinginstance) {
        $DB->delete_records('quickom_meeting_participants', array('uuid' => $meetinginstance->uuid));
    }
    $DB->delete_records('quickom_meeting_details', array('meeting_id' => $quickom->meeting_id));
 ...
 * {@link quickom_print_recent_mod_activity()}.
 *
...
/**
 * Prints single activity item prepared by {@link quickom_get_recent_mod_activity()}
 *
