...
-m, --meetingid     Quickom meeting ID
-u, --meetinguuid   Quickom meeting UUID

Example:
\$sudo -u www-data /usr/bin/php mod/quickom/cli/update_meeting_report.php --courseid=1234
";
    cli_error($help);
}
...
if (!empty($options['courseid'])) {
    // Get all Quickom meeting instances for course.
    $sql = "SELECT meeting.*
              FROM {quickom_meeting_details} meeting
         LEFT JOIN {quickom} quickom ON (quickom.id=meeting.quickomid)
             WHERE quickom.course=?";
    $meetings = $DB->get_records_sql($sql, array($options['courseid']));
} else if (!empty($options['meetingid'])) {
    // Get all Quickom meeting instances for meetingid.
    $sql = "SELECT meeting.*
              FROM {quickom_meeting_details} meeting
             WHERE meeting.meeting_id=?";
    $meetings = $DB->get_records('quickom_meeting_details',
...
$meetingtask = new mod_quickom\task\get_meeting_reports();
...
foreach ($meetings as $meeting) {
    ...
    // Query for quickom record.
    if (!isset($meetingcache[$meetingid])) {
        if (!($quickomrecord = $DB->get_record('quickom', array('meeting_id' => $meetingid), '*', IGNORE_MULTIPLE))) {
        ...
        $meetingcache[$meetingid] = $quickomrecord;
    }
    $quickomrecord = $meetingcache[$meetingid];

    // Query for course enrollment.
    if (!isset($enrollmentcache[$quickomrecord->course])) {
        $enrollmentcache[$quickomrecord->course] = $meetingtask->get_enrollments($quickomrecord->course);
    }
    list($names, $emails) = $enrollmentcache[$quickomrecord->course];

    if ($meetingtask->get_num_calls_left() < 1) {
        cli_error('Error: Quickom Report API calls have been exhausted.');
    }

    try {
        $participants = $service->get_meeting_participants($uuid, $quickomrecord->webinar);
    } catch (Exception $e) {
    ...
    foreach ($participants as $rawparticipant) {
        ...
        // Unique keys are detailsid and quickomuserid.
        if ($record = $DB->get_record('quickom_meeting_participants',
                array('detailsid' => $participant['detailsid'],
                    'quickomuserid' => $participant['quickomuserid']))) {
            ...
            if (empty($olddiff) && empty($newdiff)) {
                ...
                $DB->update_record('quickom_meeting_participants', $participant);
                $trace->output('Updated record ' . $record->id, 3);
            }
        } else {
            // Participant does not already exist.
            $recordid = $DB->insert_record('quickom_meeting_participants', $participant, false);
