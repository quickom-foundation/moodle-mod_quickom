 ...
 * @copyright  based on work by 2019 UC Regents
...
class get_meeting_reports_test extends advanced_testcase {
     ...
     * @var mod_quickom\task\get_meeting_reports
     */
    private $meetingtask;
    ...
    public function setUp() {
        ...
        $this->meetingtask = new mod_quickom\task\get_meeting_reports();
        ...
        $this->quickomdata = (object) $data;
    }

    /**
     * Make sure that format_participant() filters bad data from Quickom.
     */
    public function test_format_participant_filtering() {
        // Sometimes Quickom has a # instead of comma in the name.
        $this->quickomdata->name = 'SMITH# JOE';
        $participant = $this->meetingtask->format_participant($this->quickomdata,
                1, array(), array());
    ...
    public function test_format_participant_matching() {
        ...
        // 1) If user does not match, verify that we are using data from Quickom.
        $participant = $this->meetingtask->format_participant($this->quickomdata,
                1, array(), array());
        $this->assertEquals($this->quickomdata->name, $participant['name']);
        $this->assertEquals($this->quickomdata->user_email, $participant['user_email']);
        ...
        $user = $this->getDataGenerator()->create_user(
                array('email' => $this->quickomdata->user_email));
        ...
        // Before Quickom data is changed, should return nothing.
        $participant = $this->meetingtask->format_participant($this->quickomdata,
                1, $names, $emails);
        $this->assertNull($participant['userid']);

        // Change Quickom data and now user should be found.
        $this->quickomdata->name = strtoupper(fullname($user));
        $participant = $this->meetingtask->format_participant($this->quickomdata,
                1, $names, $emails);
        $this->assertEquals($user->id, $participant['userid']);
        $this->assertEquals($names[$participant['userid']], $participant['name']);
        // Email should match what Quickom gives us.
        $this->assertEquals($this->quickomdata->user_email, $participant['user_email']);
        ...
        $this->quickomdata->name = 'Something Else';
        ...
        // Change Quickom data and now user should be found.
        $this->quickomdata->user_email = $user->email;
        $participant = $this->meetingtask->format_participant($this->quickomdata,
                1, $names, $emails);
        $this->assertEquals($user->id, $participant['userid']);
        $this->assertEquals($names[$participant['userid']], $participant['name']);
        ...
        $participant['uuid'] = $this->quickomdata->id;
        // Set userid to a given value so we know we got a match.
        $participant['userid'] = 999;
        $recordid = $DB->insert_record('quickom_meeting_participants', $participant);

        // Should return the found entry in quickom_meeting_participants.
        $newparticipant = $this->meetingtask->format_participant($this->quickomdata,
                1, $names, $emails);
        ...
        // Email should match what Quickom gives us.
        $this->assertEquals($this->quickomdata->user_email, $newparticipant['user_email']);
    }
    ...
    public function test_format_participant_name_matching() {
        ...
        $this->quickomdata->name = fullname($users[0]);
        $participant = $this->meetingtask->format_participant($this->quickomdata,
                1, $names, $emails);
        ...
        $this->quickomdata->name = fullname($users[2]);
        $participant = $this->meetingtask->format_participant($this->quickomdata,
                1, $names, $emails);
        ...
        $this->quickomdata->name = fullname($users[4]);
        $participant = $this->meetingtask->format_participant($this->quickomdata,
                1, $names, $emails);
        $this->assertEmpty($participant['userid']);
    }
