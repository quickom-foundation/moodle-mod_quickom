...
/**
 * This file keeps track of upgrades to the quickom module
 *
...
/**
 * Execute quickom upgrade from the given old version
 *
 * @param int $oldversion
 * @return bool
 */
function xmldb_quickom_upgrade($oldversion) {
    global $DB;
    ...
    if ($oldversion < 2015071000) {
        ...
        upgrade_mod_savepoint(true, 2015071000, 'quickom');
    }
    ...
    if ($oldversion < 2015071500) {
        ...
        $starttimes = $DB->get_recordset('quickom');
        foreach ($starttimes as $time) {
            $time->start_time = strtotime($time->start_time);
            $DB->update_record('quickom', $time);
        }
        $starttimes->close();
        ...
        $DB->set_field('UPDATE {quickom} SET duration = duration*60');

        upgrade_mod_savepoint(true, 2015071500, 'quickom');
    }
    ...
    if ($oldversion < 2015071600) {
        ...
        upgrade_mod_savepoint(true, 2015071600, 'quickom');
    }
    ...
    if ($oldversion < 2015072000) {
        ...
        upgrade_mod_savepoint(true, 2015072000, 'quickom');
    }
    ...
    if ($oldversion < 2016040100) {
        ...
        // Quickom savepoint reached.
        upgrade_mod_savepoint(true, 2016040100, 'quickom');
    }

    if ($oldversion < 2018091200) {
        // Removed apiurl option from settings.
        set_config('apiurl', null, 'mod_quickom');

        // Set the starting number of API calls.
        set_config('calls_left', 2000, 'mod_quickom');

        // Set the time at which to start looking for meeting reports.
        set_config('last_call_made_at', time() - (60 * 60 * 12), 'mod_quickom');

        // Start quickom table modifications.
        $table = new xmldb_table('quickom');

        // Define field status to be dropped from quickom.
        ...
        // Define field exists_on_quickom to be added to quickom.
        $field = new xmldb_field('exists_on_quickom', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '1', 'option_audio');

        // Conditionally launch add field exists_on_quickom.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Define field uuid to be dropped from quickom.
        ...
        // Define table quickom_meeting_details to be created.
        $table = new xmldb_table('quickom_meeting_details');

        // Adding fields to table quickom_meeting_details.
        ...
        // Define table quickom_meeting_participants to be created.
        $table = new xmldb_table('quickom_meeting_participants');

        // Adding fields to table quickom_meeting_participants.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('quickomuserid', XMLDB_TYPE_INTEGER, '11', null, XMLDB_NOTNULL, null, null);
        ...
        upgrade_mod_savepoint(true, 2018091200, 'quickom');
    }

    if ($oldversion < 2018091400) {
        // Define field alternative_hosts to be added to quickom.
        $table = new xmldb_table('quickom');
        $field = new xmldb_field('alternative_hosts', XMLDB_TYPE_CHAR, '255', null, null, null, null, 'exists_on_quickom');
        ...
        // Quickom savepoint reached.
        upgrade_mod_savepoint(true, 2018091400, 'quickom');
    }

    if ($oldversion < 2018092201) {

        // Changing type of field userid on table quickom_meeting_participants to int.
        $table = new xmldb_table('quickom_meeting_participants');
        ...
        // Quickom savepoint reached.
        upgrade_mod_savepoint(true, 2018092201, 'quickom');
    }
    ...
    if ($oldversion < 2019061800) {
        ...
        // Quickom savepoint reached.
        upgrade_mod_savepoint(true, 2019061800, 'quickom');
    }

    if ($oldversion < 2020070200) {
        // Make sure start_time is not null to match install.xml.
        $table = new xmldb_table('quickom');
        $field = new xmldb_field('creator_id', XMLDB_TYPE_INTEGER, '10', null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        $field = new xmldb_field('host_key', XMLDB_TYPE_CHAR, '50', null, null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        $field = new xmldb_field('alias', XMLDB_TYPE_CHAR, '50', null, null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
